...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $con->prepare("INSERT INTO livros (Titulo, Autor, Data_lancamento, QntPaginas,Sinopse, Preco, ISBN,id_categoria) VALUES (?, ?, ?, ?, ?,?,?,?)");
        $stmt->execute([$titulo, $autor, $data_lancamento, $qtd_paginas, $sinopse, $preco, $isbn, $id_categoria]);
        ...
        if (isset($_FILES['arquivo']) && $_FILES['arquivo']['error'][0] === 0) {
            foreach ($_FILES['arquivo']['tmp_name'] as $key => $tmp_name) {
                ...
                if ($accepted) {
                    $isCapa = ($key == $capaIndex) ? 1 : 0;  //MARCA COMO CAPA SE O INDICICE CORRESPONDER A 1
...
Z            <label>Selecione as imagens:</label>
